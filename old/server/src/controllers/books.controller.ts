...
import { GetCurrentUserUtilFunction } from "./user.controller";
...
async function createBook(req: Request, res: Response) {
  try {
    const { bookName, description, tags, content } = req.body;
    /*
      bookName: string || "",
      description: string || "",
...
async function editBook(req: Request, res: Response) {
  try {
    const { bookID, mode, data } = req.body; // data is the data that will replace the already existing data
